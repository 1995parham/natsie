...
// server. Kept as a package constant rather than a parameter so unparam
// does not flag it on newApprovalServer.
const approvalSigningKey = "key"

func newApprovalServer(t *testing.T) (*Server, store.Store) {
	t.Helper()
	...
	return New(":0", st, Options{SigningKey: approvalSigningKey, Connector: connector}, log.New(io.Discard, "", 0)), st
}
...
func TestApprovalDoHappyPath(t *testing.T) {
	...
	if err := json.NewDecoder(rec.Body).Decode(&body); err != nil { //nolint:musttag // anonymous struct has explicit json tags above
